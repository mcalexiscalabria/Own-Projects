...
class HashCracker:
    ...
    def _display_progress(self, start_time, tested_count):
        ...
        sys.stdout.write(f"\r\033[K{Colors.WHITE}[+] Progress: {progress:.2f}% | H/s: {hps:,.0f} | ETA: {eta_str}{Colors.RESET}")
        sys.stdout.flush()
...
def ftp_brute_force_ui():
    ...
    def try_password(password, found_event):
        ...
        try:
            ...
            print(f"\r\033[K{Colors.GREEN}{Colors.BOLD}[SUCCESS] Login successful! Password: {password}{Colors.RESET}")
            found_event.set()
            return password
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_pass)):
                if found_event.is_set():
                    ...
                    break
                sys.stdout.write(f"\r\033[K{Colors.WHITE}[*] Progress: {((i + 1) / len(passwords)) * 100:.1f}% | Testing: {future_to_pass[future]}{Colors.RESET}")
                sys.stdout.flush()
    except KeyboardInterrupt:
    ...
    try:
        with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count() * 2) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_pass)):
                ...
                sys.stdout.write(f"\r\033[K{Colors.WHITE}[*] Progress: {((i + 1) / len(passwords)) * 100:.1f}% | Testing: {password_str}{Colors.RESET}")
                sys.stdout.flush()
                result = future.result()
                if result:
                    found_password = result
                    print(f"\r\033[K{Colors.GREEN}{Colors.BOLD}[SUCCESS] Password found: {found_password}{Colors.RESET}")
                    found_event.set()
    ...
    try:
        with open(wordlist, 'r', errors='ignore') as f:
            for password in f:
                ...
                sys.stdout.write(f"\r\033[K{Colors.TEXT}[*] Testing: {password}{Colors.RESET}")
                sys.stdout.flush()
...
def ssh_brute_force_ui():
    ...
    try:
        with open(wordlist, 'r', errors='ignore') as f:
            for password in f:
                password = password.strip()
                sys.stdout.write(f"\r\033[K{Colors.WHITE}[*] Testing: {password}{Colors.RESET}")
                sys.stdout.flush()
